...
{
    public class SearchProductQuery : IRequest<PagingData<ProductDto>>
    {
        ...
        public SearchProductQuery(string keyword, decimal? minPrice, decimal? maxPrice, int skip = 0, int take = 100)
        {
            Keyword = keyword;
            MinPrice = minPrice;
            MaxPrice = maxPrice;
    ...
    public class SearchProductQueryHandler : IRequestHandler<SearchProductQuery, PagingData<ProductDto>>
    {
        private readonly IMapper _mapper;
        IRepository<Product> _repository;
        public SearchProductQueryHandler(IRepository<Product> repository, IMapper mapper)
        {
            _repository = repository;
            _mapper = mapper;
        }

        public async Task<PagingData<ProductDto>> Handle(SearchProductQuery request, CancellationToken cancellationToken)
        {
